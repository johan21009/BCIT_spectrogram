...
def ICA_components(raw_cleaned):
    ica_cleaned = mne.preprocessing.ICA(
        n_components=64, method="picard", max_iter="auto", random_state=97, verbose='error'
    )
...
def procesar_archivo(idx):
    ...
    for event_idx, event in enumerate(filtered_events):
        ...
        ch_names = event.info['ch_names']  # Obtener nombres de canales
        for ch in ch_names:
            power = event.compute_tfr(method="multitaper", freqs=freqs, n_cycles=n_cycles, picks=ch)
            path = "/data/castrogaray-j/CH_ERDS/subject_{}_class_{}_label_{}_event_{}_channel_{}.png".format(int(pkl[8:10]), int(pkl[-5]), label, event_idx, ch)
